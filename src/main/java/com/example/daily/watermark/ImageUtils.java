...
public final class ImageUtils {
    ...
    public final static File pressImage(InputStream inputStream, File waterImgFile, int x, int y,
        ...
        try {
            ...
            // 水印文件
            Image waterImage = ImageIO.read(waterImgFile);
            int waterHeight = waterImage.getHeight(null);
            File file = ImageUtils.resize(waterImgFile, waterHeight, 6 * width / 20, false);
            // 压缩后的水印图片
    ...
    public static File resize(File f, int height, int width, boolean bb) {
        ...
        double ratio = 0; // 缩放比例
        try {
            BufferedImage bwaterMarkImg = ImageIO.read(f);
            Image itemp = bwaterMarkImg.getScaledInstance(width, height,
                    BufferedImage.SCALE_SMOOTH);
            // 计算比例
            if ((bwaterMarkImg.getHeight() > height) || (bwaterMarkImg.getWidth() > width)) {
                if (bwaterMarkImg.getHeight() > bwaterMarkImg.getWidth()) {
                    ratio = (new Integer(height)).doubleValue() / bwaterMarkImg.getHeight();
                } else {
                    ratio = (new Integer(width)).doubleValue() / bwaterMarkImg.getWidth();
                }
                AffineTransformOp op = new AffineTransformOp(
                        AffineTransform.getScaleInstance(ratio, ratio), null);
                itemp = op.filter(bwaterMarkImg, null);
            }
            if (bb) {
            ...
            ImageIO.write((BufferedImage) itemp, "png", waterTmpFile);
        } catch (IOException e) {
            e.printStackTrace();
        }
        return waterTmpFile;
